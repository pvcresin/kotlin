...
class LiteralFunctionTranslator(context: TranslationContext) : AbstractTranslator(context) {
    companion object {
        private val suspendFqName = "kotlin.coroutines.Suspend"
    }
    ...
    fun translate(
        ...
        val suspendObjectRef = if (descriptor.isCoroutineLambda && KotlinBuiltIns.isUnit(descriptor.returnType!!)) {
            val suspendObjectDescriptor = context().currentModule.builtIns.getBuiltInClassByFqName(FqName(suspendFqName))
